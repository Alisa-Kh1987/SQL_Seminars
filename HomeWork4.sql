...
SELECT u.id, firstname, lastname, COUNT(community_id) AS communities_count
...
SELECT
	communities.name AS 'Наименование сообщества',
...
*/
SELECT from_user_id, firstname, lastname, to_user_id, COUNT(to_user_id) AS count_max_mess
FROM messages
JOIN users ON users.id = messages.from_user_id
WHERE to_user_id = 1
GROUP BY from_user_id
ORDER BY count_max_mess DESC
LIMIT 1;
...
LIMIT 1;    /*можно указать "LIMIT 2" - тогда будет более четкое отображение, какой пол поставил лайков больше